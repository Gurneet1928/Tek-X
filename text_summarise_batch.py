...
final_ans = ' '
data_set = pd.read_csv("Book1.csv")
df = data_set['transcript']

def sol():
    csv_input = pd.read_csv('Book1.csv')
    csv_input['summary'] = "nan"
    csv_input.to_csv('Book1.csv', index=False)
    i=0
    global df
    dict_ans = {}
    for k in df:
        ...
        answer = summarize(text, model)
        data_set.loc[i,'summary'] = answer
        data_set.to_csv('Book1.csv', index=False)
        i = i+1
